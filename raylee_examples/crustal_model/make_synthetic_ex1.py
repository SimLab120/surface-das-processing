...
# ---------------------------
vout = np.zeros(Nf)
kk_out = np.zeros(Nf)  # Placeholder for wavenumbers
ev_out = []  # Placeholder for energy values
for i, f in enumerate(fks):
    modn = int(modnv[i])
    # Set all fluid-related arguments to None or zero as they are unused
    kk, vpk, vgk, ev = dix.raylee_lysmer(Nn, vsv, vpv, rhov, f, h, modn, Nnf, vpfv, rhofv, hfv)
    vout[i] = vpk if vtypv[i] == 0 else vgk
    # save the wavenumber (kk) and energy (ev) if needed
    kk_out[i] = kk
    ev_out.append(ev)  # Append the energy array for this frequency

# --------------------------------------
# 5. Optionally add 2.5% Gaussian noise
# --------------------------------------
add_noise = True                           # <-- Set to True if you want to add noise
